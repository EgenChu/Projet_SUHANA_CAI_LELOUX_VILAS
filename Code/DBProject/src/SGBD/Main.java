...
//			public static void main(String[] args) { Constants.PATH = args[0];
		...
		/*
		 * Constants.PATH = args[0]; List<String> typecol = new ArrayList<>();
		 * typecol.add("int"); RelDef reldef = new RelDef("première", 1, typecol, 100,
		 * 4, 819);
		 * 
		 ...
		 * 
		 * // Chargement de la headerPage 0 ByteBuffer bf =
		 * BufferManager.getInstance().getPage(p);
		 * 
		 * System.out.println("header page : " + p.getPageIdx() + bf.getInt(0) +
		 * bf.getInt(4)); System.out.println("---------");
		 * BufferManager.getInstance().freePage(p, true); // 0,0,0
		 * 
		 * List<String> values = new ArrayList<>(); values.add("127");
		 * DBManager.getInstance().insert("première", values);
		 * 
		 * values.remove("127"); values.add("234");
		 * 
		 * DBManager.getInstance().insert("première", values);
		 * 
		 ...
		 * BufferManager.getInstance().freePage(p, true); // 0,0,0
		 * 
		 * BufferManager.getInstance().flushAll(); // Page 1 ajouté
		 * 
		 * PageId page = new PageId(100, 1);
		 * 
		 * bf = BufferManager.getInstance().getPage(p); ByteBuffer bb =
		 * BufferManager.getInstance().getPage(page);
		 * System.out.println(page.getPageIdx() + " : " + bb.get(0) + " : " + bb.get(1)
		 * + " : " + bb.get(2)); System.out.println("header page : " + p.getPageIdx() +
		 * bf.getInt(0) + "+++++" + bf.getInt(4) + "+++++++" + bf.getInt(8) +
		 * bf.getInt(12)); System.out.println("---------");
